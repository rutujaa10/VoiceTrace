...
| Message Type | Action |
|---|---|
| **Voice note (sales)** | Transcribe → classify intent → extract → save → reply with summary |
| **Voice note (question)** | Transcribe → classify intent → fetch 7-day data → AI answer |
| **Text (sales)** | Classify intent → extract entities → save to ledger → reply |
| **Text (question)** | Classify intent → fetch 7-day data → AI conversational answer |
...
### 1a. 🧠 Intent Classification (Smart Routing)

Before processing any voice note or text message, an AI classification step determines whether the vendor is **logging sales** or **asking a question**.

**Flow:**
```
Voice/Text Input → Whisper transcription (if audio)
    ↓
GPT-4o-mini Intent Classifier (temp=0, max_tokens=50)
    ↓
┌── "logging" → Entity extraction → Save ledger
└── "query"  → Fetch 7-day data → AI conversational answer
```

**Classification rules:**
| Intent | Trigger | Example |
|---|---|---|
| **logging** | Vendor is TELLING about sales/expenses | "Aaj 50 samose beche" |
| **query** | Vendor is ASKING about business data | "Kal kitna kamaya?" |

**LLM config:** `temperature: 0` for deterministic classification, `json_object` response format.

**Defaults to "logging"** when uncertain — safe fallback ensures no data is lost.

---

### 1b. 📝 Text Message Processing

Vendors can now type their sales data or questions directly in WhatsApp — **no voice note required**.

**Supported text interactions:**
```
"Aaj 50 samose beche 10 rupaye mein"  →  Extracted & saved to ledger
"Kal kitna kamaya?"                    →  AI answers with 7-day data
"Mera loan score kya hai?"             →  Loan readiness + breakdown
"Sabse zyada kya bikta hai?"           →  Best-selling item analysis
```

The extraction pipeline is identical to voice — GPT-4o-mini processes the text with the same system prompt, confidence scoring, and sanitization.

---

### 1c. 🤖 AI Query Answering Engine

When a vendor asks a question (via voice, text, or the PWA chat), the system:

1. **Fetches 7-day ledger context** — all entries with items, expenses, missed profits
2. **Builds a data context string** including:
   - Vendor profile (name, category, loan score, streak)
   - 7-day summary (total revenue, expenses, profit, avg daily)
   - Daily breakdown with items sold per day
   - Top 5 items by revenue this week
3. **Generates a WhatsApp-friendly response** — short (4-5 lines), warm, in Hindi/Hinglish, with ₹ symbols

**LLM config:** `temperature: 0.5`, Roman Hindi (not Devanagari), max 300 tokens.

**Safety:** Never makes up numbers — only uses data from the context. Honestly says "data nahi hai" if missing.
...
### 10. 📦 Auto-Updated Item Catalog

Automatically tracks every unique item a vendor has ever sold, maintaining lifetime aggregates.

**Schema:**
```javascript
{
  vendor: ObjectId,
  nameNormalized: String,        // Lowercase, deduped ("samosa")
  displayName: String,           // First-seen casing ("Samosa")
  category: enum [               // Auto-classified from 200+ keywords
    'food', 'beverage', 'snack', 'sweet', 'fruit',
    'vegetable', 'dairy', 'spice', 'raw_material', 'general', 'other'
  ],
  avgPrice: Number,              // Running weighted average
  totalQuantitySold: Number,     // Lifetime total
  totalRevenue: Number,          // Lifetime total
  frequency: Number,             // Distinct days sold
  lastSoldDate: Date,
  _soldDates: [String]           // Last 90 dates (for frequency dedup)
}
```

**Key features:**
- **Atomic upserts** using MongoDB aggregation-pipeline-based `bulkWrite`
- **Exact avgPrice** = `totalRevenue / totalQuantitySold` (not a moving average)
- **Frequency dedup** — same-day multiple voice notes don't double-count
- **Auto-classification** — 200+ Indian vendor item keywords mapped to categories
- **Compound unique index** — `{vendor, nameNormalized}` prevents duplicates

**Upsert trigger:** Called automatically in both `ledger.controller.js` and `webhook.controller.js` after every `entry.save()`.

---

### 11. 📈 Advanced Weekly Pattern Detection

Analytics service that computes weekly business intelligence from the last 7 days of ledger data.

**Endpoint:** `GET /api/analytics/weekly/:vendorId`

**Returns:**
| Metric | Calculation |
|---|---|
| 🏆 **Best-selling item** | Highest total quantity across all 7-day entries |
| 🔥 **Peak revenue day** | Day with highest total revenue |
| 📉 **Missed profits** | Sum of all `missedProfits.estimatedLoss` |
| 📊 **Daily breakdown** | Revenue, expenses, profit per day |
| ⭐ **Top 5 items** | By total quantity, with revenue + avg price |

**Dashboard integration:** Three glass-cards with gradient backgrounds and colored left borders displayed on the Dashboard view.

---

### 12. 🤖 AI Voice Assistant (Chat Interface)

A full conversational chat UI in the PWA where vendors can ask business questions and get AI-powered answers.

**Backend:** `POST /api/assistant/chat` — accepts `{vendorId, message}`, re-uses the same `answerQuery` logic from the webhook with 7-day ledger context.

**Frontend features:**
- **Chat bubbles** — User (gradient purple, right-aligned) vs AI (glass card, left-aligned)
- **Typing indicator** — Three pulsing dots with "AI soch raha hai..."
- **Voice input** — 🎙️ mic button using Web Speech API (free, no server cost)
- **Quick question chips** — 6 pre-built questions shown on first load:
  ```
...
- **Speech listening indicator** with interim text preview
- **Error handling** — Graceful Hindi error bubble

**Navigation:** Accessible via 🤖 AI Assistant in the sidebar.

---

### 13. ⚡ Web Speech API (Free Browser STT)

The Record view supports **dual-mode** voice input:

| Mode | Cost | How |
|---|---|---|
| **⚡ Live Speech** (default) | **Free** | Browser's `SpeechRecognition` API → text → backend extraction |
| **🎵 Audio Upload** (fallback) | ~$0.006/min | MediaRecorder → blob upload → Whisper → extraction |

**`useVoiceRecorder` hook features:**
- `continuous: true` + auto-restart on speech gaps
- Hindi (`hi-IN`) and English (`en-IN`) language toggle
- Real-time **interim** (grey italic) + **final** (white) transcript preview
- 3-minute max duration with timer
- `isSupported` check with fallback warning for unsupported browsers
- Works in Chrome, Edge, and Android WebView

**New backend endpoint:** `POST /api/ledger/:vendorId/text` — accepts `{transcript, language}` directly, skipping Whisper entirely.

**Cost savings:** For vendors using the PWA, Live Speech mode eliminates 100% of Whisper API costs.
...
graph TB
    subgraph "User Interfaces"
        A["📱 WhatsApp<br/>Voice + Text Messages"] 
        B["🌐 PWA Dashboard<br/>React + Vite"]
        B2["🤖 AI Chat<br/>Assistant View"]
    end

    subgraph "Ingestion Layer"
        C["Twilio Webhook<br/>/api/webhook/whatsapp"]
        D["Audio Upload API<br/>/api/ledger/:id/audio"]
        D2["Text API<br/>/api/ledger/:id/text"]
        D3["Chat API<br/>/api/assistant/chat"]
    end

    subgraph "AI Processing Pipeline"
        E0["🧠 Intent Classifier<br/>logging vs query"]
        E["OpenAI Whisper<br/>Speech-to-Text"]
        E2["Web Speech API<br/>Browser STT (Free)"]
        F["GPT-4o-mini<br/>Entity Extraction"]
        F2["GPT-4o-mini<br/>Query Answering"]
    ...
    subgraph "Business Logic"
        ...
        IC["📦 Item Catalog<br/>Auto-Upsert"]
        AN["📈 Weekly Analytics<br/>Pattern Detection"]
    end
...
| **AI - Browser STT** | Web Speech API | Free in-browser transcription (Chrome/Edge) |
| **AI - NLU** | GPT-4o-mini | Entity extraction + intent classification + story generation |
| **AI - Chat** | GPT-4o-mini | Conversational query answering with business data context |
| **Messaging** | Twilio WhatsApp API | WhatsApp bot interface (voice + text) |
...
│   │   ├── controllers/
│   │   │   ├── webhook.controller.js # Twilio WhatsApp webhook (voice + text + intent)
│   │   │   ├── vendor.controller.js  # Vendor CRUD + dashboard
│   │   │   ├── ledger.controller.js  # Audio + text processing + entries
│   │   │   ├── insight.controller.js # Insights feed + CSI
│   │   │   ├── pdf.controller.js     # PDF earnings export
│   │   │   ├── analytics.controller.js # Weekly pattern detection
│   │   │   └── assistant.controller.js # AI chat assistant
│   │   ├── middlewares/
...
│   │   │   ├── Item.js               # Auto-updated item catalog
│   │   │   └── index.js              # Model barrel export
│   │   ├── routes/
│   │   │   ├── vendor.routes.js      # /api/vendors/*
│   │   │   ├── ledger.routes.js      # /api/ledger/* (audio + text)
│   │   │   ├── insight.routes.js     # /api/insights/*
│   │   │   ├── webhook.routes.js     # /api/webhook/*
│   │   │   ├── pdf.routes.js         # /api/pdf/*
│   │   │   ├── analytics.routes.js   # /api/analytics/*
│   │   │   └── assistant.routes.js   # /api/assistant/*
│   │   ├── services/
│   │   │   ├── whisper.service.js    # OpenAI Whisper transcription
│   │   │   ├── extraction.service.js # Entity extraction + intent classification + query answering
│   │   │   ├── analytics.service.js  # Weekly pattern detection engine
...
│   ├── src/
│   │   ├── api/
│   │   │   └── index.js              # Axios service layer (vendor, ledger, insight, analytics, assistant)
│   │   ├── components/
│   │   │   ├── common/
...
│   │   ├── hooks/
│   │   │   ├── useAudioRecorder.js   # MediaRecorder hook (audio blob)
│   │   │   └── useVoiceRecorder.js   # Web Speech API hook (free STT)
│   │   ├── layouts/
│   │   │   └── AppLayout.jsx         # Sidebar + mobile nav (6 items)
│   │   ├── state/
│   │   │   └── AppContext.jsx        # Context + useReducer
│   │   ├── views/
│   │   │   ├── Dashboard.jsx         # Main overview + Loan Gauge + Weekly Patterns
│   │   │   ├── Record.jsx            # Dual-mode: Live Speech (free) / Audio Upload
│   │   │   ├── Ledger.jsx            # Daily entries list
│   │   │   ├── Insights.jsx          # AI insights feed + filters
│   │   │   ├── Assistant.jsx         # AI chat interface with voice input
│   │   │   └── Login.jsx             # Phone registration
│   │   ├── index.css                 # Design system (glassmorphism)
│   │   └── app.entry.jsx             # React entry + routing (6 routes)
...
├── DETAIL.md                         # Detailed feature documentation (this file)
...
```javascript
{
  ...
  avgPrice: Number,                // Running weighted average (totalRevenue / totalQuantity)
  totalQuantitySold: Number,       // Lifetime total
  totalRevenue: Number,            // Lifetime total
  frequency: Number,               // Number of distinct days sold
  lastSoldDate: Date,
  _soldDates: [String]             // Last 90 date strings (dedup tracking, hidden field)
  // Indexes: { vendor, nameNormalized } (unique compound)
  //          { vendor, totalRevenue: -1 } (top items)
  //          { vendor, frequency: -1 } (most frequent)
}
```
...
POST   /api/ledger/:vendorId/text     # Submit pre-transcribed text (Web Speech API)
...
### Analytics Endpoints
```
GET    /api/analytics/weekly/:vendorId # Weekly pattern detection (best-seller, peak day, missed profits)
```

### Assistant Endpoints
```
POST   /api/assistant/chat             # AI chat { vendorId, message } → { reply }
```

### Webhook Endpoints
```
POST   /api/webhook/whatsapp          # Twilio incoming message (voice + text + intent)
...
┌──────────────────────────────────────────────────────────────┐
│                     INPUT SOURCES                             │
│  1. WhatsApp voice note (OGG/AMR)                            │
│  2. PWA audio recording (WebM)                               │
│  3. Web Speech API transcript (text) — FREE                  │
│  4. WhatsApp text message                                    │
│  5. AI Assistant chat message                                │
└──────────────┬───────────────────────────────────────────────┘
               │
               ▼ (audio inputs only)
┌──────────────────────────────────────────────────────────────┐
...
│     > 30% = Hindi | 5-30% = Hinglish | < 5% = English        │
│                                                               │
│  ⚡ SKIP: Web Speech API inputs bypass Whisper entirely       │
└──────────────┬───────────────────────────────────────────────┘
               │
               ▼
┌──────────────────────────────────────────────────────────────┐
│           INTENT CLASSIFICATION (NEW)                          │
│  Model: GPT-4o-mini | Temperature: 0 | Max tokens: 50        │
│  Output: { intent: "logging" | "query", confidence: 0-1 }     │
│                                                               │
│  "logging" → Entity Extraction pipeline                       │
│  "query"   → Query Answering pipeline                         │
│  Default: "logging" (safe fallback)                           │
└──────────────┬───────────────────────────────────────────────┘
               │
       ┌───────┴───────┐
       ▼ (logging)     ▼ (query)
┌──────────────┐  ┌───────────────────────────────────────────┐
│ EXTRACTION   │  │  QUERY ANSWERING (NEW)                     │
│              │  │  Model: GPT-4o-mini | Temp: 0.5 | 300 tok │
│ temp: 0.1    │  │                                            │
│ json_object  │  │  Context fed to LLM:                       │
│ 2000 tokens  │  │  ├── Vendor profile (name, category, score)│
│              │  │  ├── 7-day summary (revenue, profit, etc.) │
│ Extracts:    │  │  ├── Daily breakdown with items             │
│ • Items sold │  │  └── Top 5 items this week                  │
│ • Expenses   │  │                                            │
│ • Missed $   │  │  Rules: Roman Hindi, short (4-5 lines),    │
│              │  │  ₹ symbols, warm tone, no made-up numbers  │
└──────┬───────┘  └────────────┬──────────────────────────────┘
       │                       │
       ▼                       ▼
┌──────────────┐  ┌──────────────────────────────────────────┐
│ STRUCTURED   │  │  CONVERSATIONAL RESPONSE                  │
│ OUTPUT       │  │  Sent via WhatsApp reply                   │
│              │  │  or PWA AI Assistant chat bubble            │
│ items: []    │  └──────────────────────────────────────────┘
│ expenses: [] │
│ missed: []   │
│      ↓       │
